...
fileURL <- "https://fantasysports.yahooapis.com/fantasy/v2/team/223.l.431.t.1"
xData <- RCurl::getURL(fileURL)
doc <- XML::xmlParse(xData)



#Download fantasy football projections from Yahoo.com
# yahoo_baseurl <- "https://football.fantasysports.yahoo.com/f1/385631/players?&sort=PTS&sdir=1&status=ALL&pos=QB&stat1=S_W_2&jsenabled=1"
yahoo_baseurl <- "https://football.fantasysports.yahoo.com/f1/385631/players?&sort=PTS&sdir=1&status=ALL"
yahoo_pages <- paste0("&count=", seq(0, 150, by=25))
yahoo_pos <- list(QB="QB", RB="RB", WR="WR", TE="TE", K="K", DST="DEF")
yahoo_stats <- list(Projected = paste0("S_PW_", week), Actual = paste0("S_W_", week))
yahoo_urls <- paste0(yahoo_baseurl, yahoo_pages, "&pos=", rep(yahoo_pos, each=length(yahoo_pages)), "&stat1=", rep(yahoo_stats, each=length(yahoo_pages)),"&jsenabled=1")
